...
- [ ] **`cd-backend-dev` deploy → DEV VPS** падает с `sh: 12: set: Illegal option -o pipefail`. `appleboy/ssh-action` пробрасывает скрипт через VPS `/bin/sh` (POSIX), а в `cd-backend-dev.yml:100` стоит `set -Eeuo pipefail` — bash-only. Раньше не проявлялось, потому что build падал и до deploy не доходило (build стал зелёным после фикса lowercase IMAGE_NAME). Фикс: либо обернуть в `bash -c "..."`, либо снять `script_stop`, либо `set -eu` без pipefail. One-liner.
- [ ] **Backend Docker image** в compose: `infra/docker/backend.Dockerfile` собирается в CI (build & push image — 44s), но в `infra/compose/docker-compose.yml` пока подняты только postgres+redis. Сервисы `migrate` / `backend-listener` / `backend-worker` / `backend-api` / `backend-bot` ещё не описаны.
