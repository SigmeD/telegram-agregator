...
- [ ] **`backend-api` port-bind race на `127.0.0.1:8000/tcp` (deploy 25726662188 упал 2026-05-12):** `Error response from daemon: failed to set up container networking ... address already in use`. Старый `backend-api-1` контейнер не снимает port-binding достаточно быстро после `docker compose up -d --recreate`, новый не может стартовать. Worker / listener / bot стартуют чисто, postgres+redis healthy, migrate exited 0 — фронт стека работает, только api в port race. Фикс: в `infra/scripts/deploy.sh` перед `up` сделать `${COMPOSE[@]} rm -fs backend-api` (или `compose stop backend-api && compose rm -f backend-api`) — детерминирует освобождение порта без `down -v` (volumes сохраняются).
