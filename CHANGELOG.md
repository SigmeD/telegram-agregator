...
### Security
- **2026-07-21** PR #69 (`3b2d711`) — **Trivy HIGH-долг закрыт на `develop`.** `pyjwt 2.12.1 → 2.13.0` (CVE-2026-48526), `cryptography 46.0.7 → 48.0.1` (GHSA-537c-gmf6-5ccf), `starlette 1.0.0 → 1.3.1` (CVE-2026-54283, CVE-2026-48818). `starlette` — транзитив через `fastapi 0.136.1`; `uv lock` резолвит 1.3.1 без конфликта. `next` уже на 15.5.18 (алерты устарели, code-scanning лагает до пере-скана). Security-floor'ы прописаны в `backend/pyproject.toml` (pyjwt/cryptography напрямую, starlette — в transitive-pins блоке рядом с urllib3/mako). Верификация: 46 unit + integration зелёные, **`trivy (filesystem)` SUCCESS на develop** (прогон `29780074596`).

### Changed
- **2026-07-21** **Dependabot backlog: 13 открытых PR → 0** (был блокером Sprint 1 с 2026-05-12). Смёржено 12: react group (#64), react-hook-form (#63), lucide-react (#61), **zod 3→4** (#62, major — прямых импортов в `src` нет, ломать нечего), cryptography (#52), structlog (#32), pytest-asyncio (#33), mypy (#53), pytest (#35, конфликт pyproject разрешён вручную), docker/setup-buildx@4 (#31), docker/build-push@7 (#29), docker/login@4 (#27). Мержились раундами с `@dependabot rebase` из-за lockfile-конфликтов между PR одной экосистемы; красные оказались красными из-за **устаревших апрельских прогонов CI** (после rebase зелёные). `trivy (filesystem)` на большинстве PR — унаследованный преэкзистный fail базы (закрыт отдельно в #69), merge через `--admin`.
- **2026-07-21** PR #68 (`2adcdfa`) — **testing-группа: миграция vitest 2.1.9 → 4.1.10** (+ jsdom 25→29, @testing-library/jest-dom 6→7). Пересоздана Dependabot'ом после закрытия конфликтного #37. Root cause падения `test (vitest)`: vitest 4 требует `vite ^6||^7||^8` (peer, non-optional), а групповой бамп оставил транзитивный `vite@5.4.21` → `ERR_PACKAGE_PATH_NOT_EXPORTED: './module-runner'`. Фикс — явный пин `vite@^7` в devDependencies (vite не был прямой зависимостью — только движок vitest, проект на Next.js). `vitest.config.ts` чист (удалённых в vitest 4 coverage-опций нет). Верификация: smoke-тест (1 файл) + `tsc --noEmit` зелёные локально под vitest 4.1.10.
- **2026-07-21** PR #70 (`595dc1d`) — **ci: восстановлен `docker/setup-buildx-action@v4`.** #31 поднял buildx v3→v4, но последующие same-file merge'ы (#29 build-push, #27 login, оба трогают `cd-backend-dev.yml`) при 3-way merge несли устаревшую v3-строку и **откатили** изменение. Урок (anti-pattern): при batch-merge нескольких Dependabot-PR в один файл поздние merge'ы могут молча ревертнуть ранние — проверять итоговый контент файла, а не per-PR статус «merged». `login@v4` / `build-push@v7` уцелели.
